...
public class GenomeOracle {
  ...
  public static final String[] codons = { "ATA", "AGA", "ACA", "ATT", "AGT", "ACT", "AGG", "ACG",
      "ATC", "AGC", "ACC", "TAA", "TGA", "TCA", "TAT", "TGT", "TCT", "TGG", "TCG", "TAC", "TGC",
      "TCC", "GAA", "GTA", "GCA", "GAT", "GTT", "GCT", "GAG", "GTG", "GCG", "GAC", "GTC", "GCC",
  ...
  /**
   * Generate an array of ATGC nucleotides of a given length. Base pairs are
   * numbered from 0 to 3,088,286,400. Maximum index exceeds the capacity of an
   * int, so the index has to be a long.
   *
  ...
  public char[] getStrandSegment(long index, int length) {
    ...
    if (index >= MAX_BASE_INDEX || (index + length) >= MAX_BASE_INDEX || length < 0 || index < 0) {
    ...
    int indexChunk = (int) (index / chunkLength);
    int indexOffset = (int) (index % chunkLength);
  ...
  private void generateChunk(Random rand) {
    ...
    int skip = humanRand.nextInt(codonsPerChunk - 3); // random between 0
    // and 16
    ...
    // Since junk codons are grabbed one at a time and regions are grabbed
    // two
    ...
    // Advance the buffer chunk and set the chunk sequence. Note that we set
    // an
    // instance variable instead of returning a value, because the same
    // chunk
