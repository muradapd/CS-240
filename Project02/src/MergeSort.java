...
/**
 * Merge Sort implements the Sorter interface. This sorting method splits the
 * list into sublists. These sublists are then sorted and then merged back
 * together. It's time complexity is O(nlogn) in the every-case.
 * 
...
public class MergeSort<E> implements Sorter<E> {
  ...
  public void sort(List<E> list, Comparator<E> comp) {
    List<E> tempList = new ArrayList<E>(list); // cannot pass incoming param list to mergeSort
   ...
   */
  private void mergeSort(List<E> list, List<E> temp, int left, int right, Comparator<E> comp) {
    ...
    for (int i = left; i <= right; i++) { // Copy sublist to temp
    ...
    for (int curr = left; curr <= right; curr++) {
      ...
      } else if (comp.compare(temp.get(i1), temp.get(i2)) <= 0) { // Get smaller value
        list.set(curr, temp.get(i1++));
      } else {
        list.set(curr, temp.get(i2++));
      }
