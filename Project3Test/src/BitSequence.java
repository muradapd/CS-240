...
public class BitSequence implements Iterable<Integer>, Serializable {
   ...
   * Copy constructor.
   *
   * @param bits A bit sequence to copy.
   */
  ...
  /**
   * Append a bit to the end of this sequence. 0 will be interpreted as the bit 0,
   * all other values will be interpreted as the bit 1.
   *
  ...
  /*
   * Private helper method for doubling the size of the byte array when it becomes
   * full.
   */
  private void resize() {
   ...
   *
   * @param bits A sequence of bits to append.
   */
  ...
  /**
   * Set the value of a single bit at a specified index. 0 will be interpreted as
   * the bit 0, all other values will be interpreted as the bit 1.
   *
   * @param index The location of bit to set.
   * @param bit   The new value for that bit.
   */
  ...
  /**
   * Get the array of compacted bytes.
   *
   * @return An array of bytes containing all bits.
   */
  ...
  /**
   * Test for equality among bit sequences.
   *
   * @param other The other BitSequence object to compare to.
   *
   * @return True if all bits are the same and the lengths are equal.
   */
  @Override
  ...
  /**
   * The hashcode for a BitSequence is constructed by xoring each subsequent byte
   * at a rotating offset.
   *
   * @return A hash value for the bits.
   */
  @Override
  public int hashCode() {
    int result = 0;
   ...
   *
   * @return The bits as a String object.
   */
  @Override
  ...
  /**
   * Iterator for the bits as Integers.
   *
   * @return An Iterator that returns each bit as an Integer.
   */
  @Override
   ...
   */
  private void trim() {
    byte[] newBytes = new byte[numBits / 8 + 1];
   ...
   *
   * @param out The output stream (typically a file) to write to.
   * 
